...
from engine import train_one_epoch, evaluate_nosbi
from detectors.vitb_stada_dfg_bfg_detector import vitbstDetector
from networks.nce_loss import ContrastiveLoss
import torch.nn as nn

import utils

from video_dataset_combine import VideoDataSet, VideoDataSet_nosbi
from video_dataset_aug import get_augmentor, build_dataflow, build_dataflow_nosbi
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_txt_dir', type=str,default='./datasets/FaceForensics++/data_txt', help='path to text of dataset')
    parser.add_argument('--data_dir', type=str,default="./datasets/FaceForensics++", help='path to dataset')
    parser.add_argument('--dataset', default='ffpp',
                        choices=list(DATASET_CONFIG.keys()), help='path to dataset file list')
    #------------------------add begin-------------------------------------
    parser.add_argument('--val_data_txt_dir', type=str,default='./datasets/FaceForensics++/data_txt', help='path to text of val dataset')
    parser.add_argument('--val_data_dir', type=str,default="./datasets/FaceForensics++", help='path to val dataset')
    parser.add_argument('--val_dataset', default='ffpp',
                        choices=list(DATASET_CONFIG.keys()), help='path to val dataset file list')
    parser.add_argument('--save_all_ckpt', action='store_true', help='whether save all ckpt')
    #------------------------add end-------------------------------------
    parser.add_argument('--duration', default=32, type=int, help='number of frames')
    ...
    # Model parameters
    parser.add_argument('--model', default='vitb_st', type=str, metavar='MODEL',
...
def main(args):
    ...
    num_classes, train_list_name, _, test_list_name, filename_seperator, image_tmpl, filter_video, label_file = get_dataset_config(
        args.dataset, args.use_lmdb)
    num_classes_val, _, val_list_name, test_list_name, filename_seperator_val, image_tmpl_val, filter_video_val, label_file_val = get_dataset_config(
        args.val_dataset, args.use_lmdb)
    ...
    model = vitbstDetector(args.duration, args.num_clips)
    ...
    nce_criterion = ContrastiveLoss(temperature=args.temperature)
    ...
    dataset_train = video_data_cls(args.data_dir, train_list, args.duration, args.frames_per_group,
                                ...
                                modality=args.modality, image_size=args.input_size, image_tmpl=image_tmpl,
    ...
    video_data_cls_val = VideoDataSet_nosbi
    val_list = os.path.join(args.val_data_txt_dir, val_list_name)
    val_augmentor = get_augmentor(False, args.input_size, mean, std, args.disable_scaleup,
                                ...
                                scale_range=args.scale_range, num_clips=args.num_clips, num_crops=args.num_crops,cut_out = False, dataset=args.val_dataset)
    dataset_val = video_data_cls_val(args.val_data_dir, val_list, args.duration, args.frames_per_group,
                                num_clips=args.num_clips,
                                modality=args.modality, image_size=args.input_size, image_tmpl=image_tmpl_val,
                                dense_sampling=args.dense_sampling,
                                transform=val_augmentor, is_train=False, test_mode=False,
                                seperator=filename_seperator_val, filter_video=filter_video_val)

    data_loader_val = build_dataflow_nosbi(dataset_val, is_train=False, batch_size=args.batch_size,
                                    workers=args.num_workers, is_distributed=args.distributed)
    ...
    if args.initial_checkpoint:
        ...
        model_without_ddp.vitb.load_state_dict(checkpoint['model'], strict=False)
        print(f'load pretrained model from {args.initial_checkpoint}')
    ...
    if args.eval:
        test_stats = evaluate_nosbi(data_loader_val, model, device, num_tasks, distributed=args.distributed, amp=args.amp, num_crops=args.num_crops, num_clips=args.num_clips, temperature=args.temperature)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, nce_criterion, data_loader_train,args.num_clips,
