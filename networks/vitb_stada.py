...
from .feat_style import FeatureStylization
from .bfg import bfg
...
class VisionTransformer_dfg_bfg(nn.Module):
    def __init__(self,
        ...
        self.ln_patch_post = LayerNorm(width)
        self.fuse_ln_post = LayerNorm(width)
        self.bfg_ln_post = LayerNorm(width)
    ...
    def forward(self, x: torch.Tensor, num_clips, train: bool, is_bfg: bool):
        ...
        patch_feats = x[:,1:,:].reshape(-1, x.size(-1)) #[bnt*(L-1), D]
        ...
        # style fusion
        if train:
            x_fuse = FeatureStylization(x.clone(), num_clips)
            x_fuse = x_fuse.mean(dim=1)
            fuse_feats = self.fuse_ln_post(x_fuse) #[bn,768]
            fuse_feats = self.dropout(fuse_feats)
            fuse_pred = self.fc(fuse_feats)
            if is_bfg:
                x_bfg = bfg(x.clone(), num_clips)
                x_bfg = x_bfg.mean(dim=1)
                bfg_feats = self.bfg_ln_post(x_bfg) #[b//2*n,768]
                bfg_feats = self.dropout(bfg_feats)
                bfg_pred = self.fc(bfg_feats)

        x = x.mean(dim=1)
        feats = self.ln_post(x) #[bn,768]
        feats = self.dropout(feats)
        pred = self.fc(feats)
        
        patch_feats = self.ln_patch_post(patch_feats)
        patch_feats = self.dropout(patch_feats)
        patch_pred = self.fc(patch_feats) #[bnt*(L-1), 2]

        if train:
            if is_bfg:
                feats = torch.cat([feats, fuse_feats, bfg_feats], dim=0)
                pred = torch.cat([pred, fuse_pred, bfg_pred], dim=0)
            else:
                feats = torch.cat([feats, fuse_feats], dim=0)
                pred = torch.cat([pred, fuse_pred], dim=0)

        return pred, feats, patch_pred


def clip_vit_base_patch16_adapter24x384_dfg_bfg(**kwargs):
    model = VisionTransformer_dfg_bfg(
    ...
    checkpoint = torch.jit.load("./pretrained/ViT-B-16.pt", map_location='cpu')
