...
def get_source_transforms():
    return A.Compose([
						A.RGBShift((-20,20),(-20,20),(-20,20),p=0.5),
...
# data aug
def get_transform(size=224, is_train=True):
    if is_train:
        return A.Compose([           
            ...
            A.Resize(height=size, width=size, interpolation=cv2.INTER_LINEAR, always_apply=True),
...
class GroupCenterCrop(object):
    def __init__(self, size):
        ...
        self.transform = A.CenterCrop(height=self.height, width=self.width, always_apply=True)
...
class GroupRandomHorizontalFlip(object):
    ...
    def __call__(self, img_group, is_flow=False):
        ...
        ret = [transform(image=img)['image'] for img in img_group]
        return ret
...
class GroupNormalize(object):
    def __init__(self, mean, std, threed_data=False):
        ...
        if self.threed_data:
            ...
            self.mean = torch.FloatTensor(mean).view(1, len(mean), 1, 1)
            self.std = torch.FloatTensor(std).view(1, len(std), 1, 1)
        else:
            self.mean = mean
            self.std = std
...
class GroupScale(object):
    ...
    def resize_image(self, img):
        resize = A.Resize(height=self.size, width=self.size, interpolation=self.interpolation, always_apply=True)
        return resize(image=img)['image']

    def __call__(self, img_group):
        return [self.resize_image(img) for img in img_group]
...
class GroupMultiScaleCrop(object):
    ...
    def __call__(self, img_group):
        ...
        crop_img_group = [img[offset_h:(offset_h + crop_h + 1), offset_w:(offset_w + crop_w + 1)] for img in img_group]
        ret_img_group = [cv2.resize(img, self.input_size, interpolation=cv2.INTER_LINEAR)
                         for img in crop_img_group]
        ...
        #     im_size = img.shape
        #     crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(im_size)
        #     crop = A.Crop(x_min=offset_w, y_min=offset_h, x_max=offset_w + crop_w, y_max=offset_h + crop_h, always_apply=True)
        #     resize = A.Resize(height=self.input_size[1], width=self.input_size[0], interpolation=self.interpolation, always_apply=True)
        #     transform = A.Compose([crop, resize])
        #     ret_img_group.append(transform(image=img)['image'])
