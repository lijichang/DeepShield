...
def detect(img_list, output_path, start_index=0, resize=1):
...
def extract_frames(data_path, interval=1):
    ...
    frame_num = 0
    frames = list()
    while reader.isOpened() and len(frames) < args.max_frames:
...
def extract_method_videos(data_path, interval):
    ...
    images_path = join(data_path, 'frames')
    ...
    for video in tqdm(os.listdir(videos_path)):
        ...
        output_folder = join(images_path, image_folder)
        existing_frames = len([f for f in os.listdir(output_folder) if f.endswith('.png')])
        
        if existing_frames >= args.max_frames:
            continue
        
        try:
            image_list = extract_frames(join(videos_path, video), interval)
            detect(image_list[existing_frames:], output_folder, start_index=existing_frames)
...
if __name__ == '__main__':
    ...
    p.add_argument('--max_frames', default=1000, type=int, help='maximum frames per video')
    ...
    # use resnet-50
    cfg = cfg_re50
    pretrained_weights = 'Resnet50_Final.pth'
