...
from engine import test_nosbi
from detectors.vitb_stada_dfg_bfg_detector import vitbstDetector
#import simclr
import utils

from video_dataset_combine import VideoDataSet_nosbi
from video_dataset_aug import get_augmentor, build_dataflow_nosbi
from video_dataset_config import get_dataset_config, DATASET_CONFIG
from distort.distortion import color_contrast, block_wise, gaussian_blur, jpeg_compression, color_saturation, gaussian_noise_color, get_distortion_parameter
...
def get_args_parser():
    ...
    parser.add_argument('--model_name',default="vitb_st")
    ...
    # for distort
    parser.add_argument('--distort_func', default=None, type=str) # choices=['bw', 'cc', 'jc', 'gb', 'cs']
    parser.add_argument('--distort_level', default=0, type=int)
    ...
    parser.add_argument('--initial_checkpoint', type=str, default=None, help='path to the pretrained model')
    # ------------------add---------------------
    parser.add_argument('--test_ckpt_dir', type=str, default='', help='dir to the trained model')
    # ------------------add end---------------------
...
# create data loaders w/ augmentation pipeiine
    video_data_cls = VideoDataSet_nosbi

    if args.distort_func == None:
        distort_func = None
    elif args.distort_func == 'cc':
        distort_func = color_contrast
    elif args.distort_func == 'bw':
        distort_func = block_wise
    elif args.distort_func == 'gb':
        distort_func = gaussian_blur
    elif args.distort_func == 'jc':
        distort_func = jpeg_compression
    elif args.distort_func == 'cs':
        distort_func = color_saturation
    elif args.distort_func == 'gnc':
        distort_func = gaussian_noise_color
        
    if args.distort_func:
        distort_param = get_distortion_parameter(args.distort_func, args.distort_level)
    else:
        distort_param = 0
    ...
    dataset_val = video_data_cls(args.data_dir, val_list, args.duration, args.frames_per_group,
                                ...
                                modality=args.modality, image_size=args.input_size,
                                dense_sampling=args.dense_sampling,
                                image_tmpl=image_tmpl,
                                transform=val_augmentor,
                                is_train=False, test_mode=False,
                                seperator=filename_seperator, filter_video=filter_video,
                                distort_func=distort_func, distort_param=distort_param)
    ...
    max_acc = 0.
    for path in sorted(os.listdir(args.test_ckpt_dir)):
        if path.endswith('best.pth'):
            checkpoint = torch.load(os.path.join(args.test_ckpt_dir, path), map_location='cpu')
            vitb_statedict = checkpoint['model']
            model_without_ddp.load_state_dict(vitb_statedict, strict=False)

            print(f'ckpt: {path}')
            state = test_nosbi(data_loader_val, model, device, num_tasks, distributed=args.distributed, amp=args.amp, num_crops=args.num_crops, num_clips=args.num_clips)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {state['acc1']:.1f}%")
            max_acc = max(max_acc, state['acc1'])
